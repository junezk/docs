...
Go modules 是 Go 语言的依赖解决方案，发布于 Go1.11，成长于 Go1.12，丰富于 Go1.13，正式于 Go1.14 推荐在生产上（ready for production）使用。
...
我们有提到 Go modules 的解决的问题之一就是“淘汰”掉 GOPATH，但是 GOPATH 又是什么呢，为什么不再推荐 GOPATH 的模式了呢？
...
在 GOPATH 的 `$GOPATH/src` 下进行 `.go` 文件或源代码的存储，我们可以称其为 GOPATH 的模式，这个模式，看起来好像没有什么问题，那么为什么我们要弃用呢？因为**GOPATH 模式下没有版本控制的概念**，具有致命的缺陷，至少会造成以下问题：
...
这时候社区中逐渐的涌现出了大量的依赖解决方案，其中包括我们所熟知的 vendor 目录的模式，以及曾经一度被认为是“官宣”的 dep 的这类依赖管理工具。随着 Russ Cox 与 Go 团队中的其他成员不断深入地讨论，发现dep 的一些细节似乎越来越不适合 Go，最终演变为我们现在所见到的 Go modules，也在 Go1.11 正式进入了 Go 的工具链。
...
| 命令            | 作用                                                         |
| --------------- | ------------------------------------------------------------ |
| go mod init     | 创建一个使用Go Modules的项目，并生成 go.mod 文件             |
| go mod download | 下载 go.mod 文件中指明的所有依赖到本地缓存$GOPATH/pkg/mod/cache |
| go mod tidy     | 整理现有的依赖（添加缺失的，移除不再使用的）                 |
| go mod graph    | 查看现有的依赖结构                                           |
| go mod edit     | 编辑 go.mod 文件（用编辑器更方便）                           |
| go mod vendor   | 导出所有的依赖到vendor目录                                   |
...
![image](Go Modules 终极入门.assets/170851e39ca3b20d)
...
![image](Go Modules 终极入门.assets/170851e39b086b76)
...
![image](Go Modules 终极入门.assets/170851e3a2473ad7)
...
![image](Go Modules 终极入门.assets/170851e3a20d3324)
...
![image](Go Modules 终极入门.assets/170851e44fd1d0fb)
...
![image](Go Modules 终极入门.assets/170851e453a9137d)
