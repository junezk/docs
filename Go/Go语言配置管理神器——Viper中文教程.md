...
[Viper](https://link.zhihu.com/?target=https%3A//github.com/spf13/viper)是适用于Go应用程序的完整配置解决方案。它被设计用于在应用程序中工作，并且可以处理所有类型的配置需求和格式。
...
Viper 具有绑定到标志的能力。具体来说，Viper支持[Cobra](https://link.zhihu.com/?target=https%3A//github.com/spf13/cobra)库中使用的`Pflag`。
...
Viper使用[crypt](https://link.zhihu.com/?target=https%3A//github.com/bketelsen/crypt)从K/V存储中检索配置，这意味着如果你有正确的gpg密匙，你可以将配置值加密存储并自动解密。加密是可选的。

你可以将远程配置与本地配置结合使用，也可以独立使用。

`crypt`有一个命令行助手，你可以使用它将配置放入K/V存储中。`crypt`默认使用在[http://127.0.0.1:4001](https://link.zhihu.com/?target=http%3A//127.0.0.1%3A4001/)的etcd。
...
Viper在后台使用[github.com/mitchellh/mapstructure](https://link.zhihu.com/?target=https%3A//github.com/mitchellh/mapstructure)来解析值，其默认情况下使用`mapstructure`tag。
